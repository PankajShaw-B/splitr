...
export const getGroupExpenses = query({
  ...
  handler: async (ctx, { groupId }) => {
    ...
    /* Example:
       Expense 1: user1 paid $60, split equally among all 3 users ($20 ecah)
    ...
    */
    for (const exp of expenses) {
      ...
      for (const split of exp.splits) {
        ...
        // update totals: increase payer's balnace, decrease debtor's balance
    ...
    /* Example:
       ...
       ledger = {
        ...
        This means user2 owes user1 only $10 and user3 still owes user1 $20
    */
    ...
    After simplification
        ...
        user2 owes user3 $15
        user3 owes user1 $0
    This reduces the circular debt
    */

    ids.forEach((a) => {
      ids.forEach((b) => {
        if (a >= b) return; // visit each unordered pair once
        const diff = ledger[a][b] - ledger[b][a]; // calculate the net debt between two users
        if (diff > 0) {
          ledger[a][b] = diff;
          ledger[b][a] = 0;
        } else if (diff < 0) {
    ...
    return {
     // Group Info
      ...
      settlements, // All settlemnets in this group
